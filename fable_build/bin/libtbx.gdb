...
LIBTBX_DISPATCHER_NAME="libtbx.gdb"
...
if [ -n "$LIBTBX__VALGRIND_FLAG__" ]; then
  exec $LIBTBX_VALGRIND "$LIBTBX_BUILD/../fable_sources/libtbx/command_line/gdb.sh" "$@"
elif [ $# -eq 0 ]; then
  exec "$LIBTBX_BUILD/../fable_sources/libtbx/command_line/gdb.sh"
else
  exec "$LIBTBX_BUILD/../fable_sources/libtbx/command_line/gdb.sh" "$@"
fi
