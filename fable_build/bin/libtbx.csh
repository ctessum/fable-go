...
LIBTBX_DISPATCHER_NAME="libtbx.csh"
...
if [ -n "$LIBTBX__VALGRIND_FLAG__" ]; then
  exec $LIBTBX_VALGRIND "$LIBTBX_BUILD/../fable_sources/libtbx/command_line/csh.sh" "$@"
elif [ $# -eq 0 ]; then
  exec "$LIBTBX_BUILD/../fable_sources/libtbx/command_line/csh.sh"
else
  exec "$LIBTBX_BUILD/../fable_sources/libtbx/command_line/csh.sh" "$@"
fi
