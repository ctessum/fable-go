...
LIBTBX_DISPATCHER_NAME="libtbx.tcsh"
...
if [ -n "$LIBTBX__VALGRIND_FLAG__" ]; then
  exec $LIBTBX_VALGRIND "$LIBTBX_BUILD/../fable_sources/libtbx/command_line/tcsh.sh" "$@"
elif [ $# -eq 0 ]; then
  exec "$LIBTBX_BUILD/../fable_sources/libtbx/command_line/tcsh.sh"
else
  exec "$LIBTBX_BUILD/../fable_sources/libtbx/command_line/tcsh.sh" "$@"
fi
